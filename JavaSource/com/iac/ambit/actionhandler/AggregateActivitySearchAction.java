...
public class AggregateActivitySearchAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			request.getSession().setAttribute("ACTIVITIES_IN_REQUEST",
					ACTIVITIES);
			int total = 0;
			for (int i = 0; i < ACTIVITIES.length; i++) {

				total += Integer.parseInt(ACTIVITIES[i].getAggregate());
			}
			request.getSession().setAttribute("AGGREGATE_TOTAL",
					String.valueOf(total));

		} catch (Exception e) {
			ErrorsUtility.AddGlobalErrorToActionMessages(request,
			"general.operationfailed");
			Tracer.exception("AggregateActivitySearchAction", "ActivitySearchAction",
					"Exception occured Activity Search Action : ", e);
			path = "failure";
