...
/**
 * Title: This utility class includes several util methods regarding to Date
 * management Description: Copyright: Copyright (c) 2002 Company: Avanza
 * Solutions (Pvt) Ltd.
 * 
...
public class DateUtils {
	...
	private static final String DATE_TIME_UNFORMAT="yyyyMMddHHmmss";
	 ...
	 * 
	 * @param dtDate
	 *            A {@link java.util.Date} object. The date to format
	 * @return A string representing the input date in the format specified by
	 *         DATE_FORMAT
	 * @version 0.1
	 */
	...
	public static String getCurrentDateSeparatedByChar(char separator) {
		...
		String dateValue = calendar.get(Calendar.DAY_OF_MONTH)
				+ String.valueOf(separator) + calendar.get(Calendar.MONTH)
				+ String.valueOf(separator) + calendar.get(Calendar.YEAR);
	...
	/**
	 * Overloaded timeToString funciton which takes date and date format as
	 * param
	 */
	public static String timeToString(Date dtDate, String timeFormat) {
	...
	/**
	 * Overloaded dateToString funciton which takes date and date format as
	 * param
	 */
	...
	/**
	 * Returns a string representing the given date and time in the format
	 * DATETIME_FORMAT
	 * 
	 * @param dtDate
	 *            A {@link java.util.Date} object. The date to format
	...
	/**
	 * Parses a string formatted as specified by DATE_FORMAT and returns the
	 * corresponding {@link java.util.Date} object.
	 * 
	 * @param sDate
	 *            A string in the format specified by DATE_FORMAT
	 * @return A {@link java.util.Date} object. The date represented by the
	 *         given string. Returns null if the string is not in the correct
	 *         format
	 * @version 0.1
	 */
	...
	/**
	 * Parses a string formatted as specified by DATETIME_FORMAT and returns the
	 * corresponding {@link java.util.Date} object.
	 * 
	 * @param sDate
	 *            A string in the format specified by DATETIME_FORMAT
	 * @return A {@link java.util.Date} object. The date represented by the
	 *         given string. Returns null if the string is not in the correct
	 *         format
	 * @version 0.1
	...
	public static Date stringToDatetime(String sDate, String dateFormat) {
		...
		try {
			...
			Tracer.traceOut(Tracer.Debug_Level, "DateUtils",
					"stringToDatetime()",
					"Exception occured while converting string to date/time: "
							+ e);
	 ...
	 * 
	 * @param sDate
	 *            The {@link java.util.Date} to increment one day.
	 ...
	 * 
	 * @param sDate
	 *            The {@link java.util.Date} to decrement one day.
	...
	public static String getCurrentTime() {
		...
		String timeValue = formatRecordNumber(calendar
	...
	public static int getAddedTimeStamp() {
		...
		dateValue = dateValue + calendar.get(Calendar.HOUR)
	...
	public static String getCurrentDateForATMBatch() {
		...
		String dateValue = calendar.get(Calendar.YEAR)
				+ ""
				+ AmbitUtility.formatRecordNumber(
						(calendar.get(Calendar.MONTH) + 1), 2)
				+ ""
				+ AmbitUtility.formatRecordNumber(calendar
	...
	public static String getFileNameStamp() {
		...
		String dateValue = "" + calendar.get(Calendar.DAY_OF_MONTH)
				+ (calendar.get(Calendar.MONTH) + 1)
				+ calendar.get(Calendar.YEAR);
		String timeValue = "" + calendar.get(Calendar.HOUR)
				+ calendar.get(Calendar.MINUTE) + calendar.get(Calendar.SECOND);
	 ...
	 * @param dtFrom
	 *            A {@link java.util.Date} object.
	 * @return True if the current date is between toDate and fromDate.
	 *         Otherwise false.
	 * @version 0.1
	 * @since 14/10/2002
	 */
	...
	public static String checkDateBefore(String date) {
		...
		} else if (stringToDate(date).before(
				DateUtils.stringToDate("31/12/1900"))) {
			return "";

		}

		return DateUtils
				.dateToString(stringToDate(date), DateUtils.DATE_FORMAT);
	...
	public static String getGreetingMoment(Date dt) {
		...
		// jazimagh : 1386/05/23
		/*
		 * if ( ( c.get( c.HOUR_OF_DAY ) >= Integer.parseInt(
		 * Config.getProperty("GREET_MORNING_START") ) ) && ( c.get(
		 * c.HOUR_OF_DAY ) < Integer.parseInt(
		 * Config.getProperty("GREET_MORNING_END") ) ) ) { return
		 * Config.getProperty("GREET_MORNING"); }
		 * 
		 * if ( ( c.get( c.HOUR_OF_DAY ) >= Integer.parseInt(
		 * Config.getProperty("GREET_AFTERNOON_START") ) ) && ( c.get(
		 * c.HOUR_OF_DAY ) < Integer.parseInt(
		 * Config.getProperty("GREET_AFTERNOON_END") ) ) ) { return
		 * Config.getProperty("GREET_AFTERNOON"); }
		 * 
		 * if ( ( c.get( c.HOUR_OF_DAY ) >= Integer.parseInt(
		 * Config.getProperty("GREET_EVENING_START") ) ) || ( c.get(
		 * c.HOUR_OF_DAY ) < Integer.parseInt(
		 * Config.getProperty("GREET_EVENING_END") ) ) ) { return
		 * Config.getProperty("GREET_EVENING"); }
		 */
	...
	public static Date getDateStringForAmbit(String sDate) {
		...
		if (sDate.length() >= 10) {
			c.set(Calendar.HOUR_OF_DAY, Integer
					.parseInt(sDate.substring(8, 10)));

			if (sDate.length() >= 12) {
				c.set(Calendar.MINUTE, Integer
						.parseInt(sDate.substring(10, 12)));

				if (sDate.length() >= 14) {
					c.set(Calendar.SECOND, Integer.parseInt(sDate.substring(12,
							14)));
				}
			}
		}
	...
	public static Date getDateTimeStringForAmbit(String sDate) {
		Date rtDate = new Date();
		Calendar c = Calendar.getInstance();
		...
		if (m.matches()) {
			c.set(Calendar.YEAR, (Integer.parseInt(sDate.substring(0, 4))));
			c
					.set(Calendar.MONTH, (Integer.parseInt(sDate
							.substring(4, 6)) - 1));
			c.set(Calendar.DAY_OF_MONTH, Integer
					.parseInt(sDate.substring(6, 8)));

			if (sDate.length() >= 10) {
				c.set(Calendar.HOUR_OF_DAY, Integer.parseInt(sDate.substring(8,
						10)));

				if (sDate.length() >= 12) {
					c.set(Calendar.MINUTE, Integer.parseInt(sDate.substring(10,
							12)));

					if (sDate.length() >= 14) {
						c.set(Calendar.SECOND, Integer.parseInt(sDate
								.substring(12, 14)));
					}
				}
			}
		} else {
			throw new AmbitException("DateUtils", "getDateTimeStringForAmbit",
					"Incorrect Date Format " + sDate);
	...
	public static Date getYesterDate(int numberOfDays) {
		...
		c.set(Calendar.DAY_OF_YEAR,
				(c.get(Calendar.DAY_OF_YEAR) - numberOfDays));
	 ...
	 * getDateTimeForUNISON( "01/12/2004" ) );
	 *  }
	 */
	...
	public static Date getDateTimeStringForSPG(String sDate) {
		Date rtDate = new Date();
		Calendar c = Calendar.getInstance();
		...
		if (m.matches()) {
			...
			throw new AmbitException("DateUtils", "getDateTimeStringForSPG",
					"Incorrect Date Format " + sDate);
	...
	public static String getYear(String curDate) {
		String yearValue = "";
		Calendar calendar = Calendar.getInstance();
		yearValue = AmbitUtility.leadingZero(Integer.toString(calendar
				.get(Calendar.YEAR)), 4);
	...
	public static String getCurLDate() {
		...
		String dateValue = AmbitUtility.leadingZero(Integer.toString(calendar
				.get(Calendar.YEAR)), 4)
				+ AmbitUtility.leadingZero((Integer.toString(calendar
	...
	public static String completeLMonthDay(String monthDay) {
		...
		int m = Integer.parseInt(AmbitUtility.strMid(monthDay, 0, 2))
				- Integer.parseInt(AmbitUtility.strMid(curMonth, 0, 2));
	...
	public static String formatedLdate(String latinDate) {
		...
		String fDate = null;
		fDate = AmbitUtility.strMid(latinDate, 0, 4) + CHR_SLASH
				+ AmbitUtility.strMid(latinDate, 4, 2) + CHR_SLASH
				+ AmbitUtility.strMid(latinDate, 6, 2);

		return fDate;

	}

	public static long getDiferenceTime(String formatTime, String time1)
	...
	public static long stringToTime(String sDate) {
		SimpleDateFormat dateFormatter = new SimpleDateFormat(DATE_TIME_UNFORMAT);
